...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT5000IFMN = 3,
    ONLY_NONDENOMINATED_NOT5000IFMN = 4, // ONLY_NONDENOMINATED and not 5000 BARA at the same time
    ONLY_5000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBARA send
enum ZerocoinSpendStatus {
    ZBARA_SPEND_OKAY = 0,                            // No error
    ZBARA_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBARA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBARA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBARA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBARA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBARA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBARA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBARA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBARA_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBARA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBARA_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBARA_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBARA_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBARA_SPENT_USED_ZBARA = 14,                      // Coin has already been spend
    ZBARA_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};

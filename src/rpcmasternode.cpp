...
UniValue listmasternodes(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH (PAIRTYPE(int, CMasternode) & s, vMasternodeRanks) {
        ...
        if (mn != NULL) {
			...
			obj.push_back(Pair("coin", strCoin));
...
UniValue masternodeconnect(const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:59253\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6:59253\""));
...
UniValue startmasternode (const UniValue& params, bool fHelp)
{
    ...
    if (strCommand == "all" || strCommand == "many" || strCommand == "missing" || strCommand == "disabled") {
        ...
        BOOST_FOREACH (CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            ...
            bool result = activeMasternode.Register(mne.getIp(), mne.getPrivKey(), mne.getTxHash(), mne.getOutputIndex(), mne.getMNcoin(), errorMessage);
...
UniValue getmasternodestatus (const UniValue& params, bool fHelp)
{
    ...
    if (pmn) {
		...
		mnObj.push_back(Pair("coin", activeMasternode.GetMNcoin()));
        return mnObj;
    }
